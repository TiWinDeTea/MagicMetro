	...
	\chapter{Classes et liaisons}
		\section{Modèle}
			...
			\subsection{Train}
				...
				\\Le Train possèdera aussi son état. L'état du train est une interface nommée \jinterface{TrainState} qui sera implémenté par \jinterface{InMovingState} et par \jinterface{AtStationState}. Ainsi, quand on sera dans une station, l'état du train sera AtStationState et quand ils se déplacera sur la ligne, son état sera une InMovingState. CE choix a été fait pour qu'on appelle la fonction live dans la boucle de jeu sans se soucier de l'endroit où est le train.
			\subsection{Station}
