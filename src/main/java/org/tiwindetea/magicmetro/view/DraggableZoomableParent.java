	...
	public DraggableZoomableParent() {
		...
		setOnScroll(new EventHandler<ScrollEvent>() {
			...
			public void handle(ScrollEvent event) {
				...
				double dx = (event.getSceneX() - (DraggableZoomableParent.this.getBoundsInParent()
				  .getWidth() / 2 + localToScene(getBoundsInLocal()).getMinX()));
				double dy = (event.getSceneY() - (DraggableZoomableParent.this.getBoundsInParent()
				  .getHeight() / 2 + localToScene(getBoundsInLocal()).getMinY()));
